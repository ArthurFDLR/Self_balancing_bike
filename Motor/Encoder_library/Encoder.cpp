...
EncoderQuad::EncoderQuad(uint8_t pinEncoderChA, uint8_t pinEncoderChB, float paramExpFilter, uint8_t encoderCountRev)
{
    ...
    attachInterrupt(digitalPinToInterrupt(_pinEncoderAInterrupt), ISR_updateEncoder, CHANGE);

    _motorTimePrev = millis();
}

EncoderQuad::ISR_updateEncoder() //Direct access to register to optimize time consumption
{
